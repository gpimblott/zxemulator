...
TEST(ProcessorTest, Check8And16BitValues) {
    ...
    // We expect the bytes to be 'logically' reversed due to the little endian ordering
    EXPECT_EQ(registers.HL, wordOne);
}

/**
 * Check that rotating a bit between the 8 bit registers results in the expected 16 bit value
 */
