...
void Processor::executeFrame() {
  // Auto-Turbo Mode: If tape is playing, uncap framerate
  bool tapePlaying = state.tape.isPlaying();
  if (tapePlaying != turboEnabled) {
    turboEnabled = tapePlaying;
    if (screen) {
      screen->setTurboMode(turboEnabled);
    }
  }

  int frameIterations = turboEnabled ? 4 : 1;
