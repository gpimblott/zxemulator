...
// Fix ESP32C3 IDF bug for missing definition (VSPI/FSPI only tested at the
// moment)
#ifndef REG_SPI_BASE
...
(C3 only SPI2 port is available, SPI0 & SPI1 are dedicated to internal flash
memory) Confusingly in ESP-IDF enumerations are: SPI1_HOST = 0,  ///< actually
SPI0 SPI2_HOST = 1,  ///< actually SPI1 SPI3_HOST = 2,  ///< actually SPI2
*/
...
#define DC_C                                                                   \
  GPIO.out_w1tc.val =                                                          \
      (1 << (TFT_DC - 32)) //;GPIO.out_w1tc.val = (1 << (TFT_DC - 32))
#define DC_D                                                                   \
  GPIO.out_w1ts.val =                                                          \
      (1 << (TFT_DC - 32)) //;GPIO.out_w1ts.val = (1 << (TFT_DC - 32))
#endif
#elif (TFT_DC >= 0)
...
#else
#define DC_C                                                                   \
  GPIO.out_w1tc.val = (1 << TFT_DC) //;GPIO.out_w1tc.val = (1 << TFT_DC)
#define DC_D                                                                   \
  GPIO.out_w1ts.val = (1 << TFT_DC) //;GPIO.out_w1ts.val = (1 << TFT_DC)
#endif
#else
#define DC_C
#define DC_D
...
#define CS_H                                                                   \
  GPIO.out_w1ts.val =                                                          \
      (1 << (TFT_CS - 32)) //;GPIO.out_w1ts.val = (1 << (TFT_CS - 32))
#endif
#elif (TFT_CS >= 0)
#ifdef RPI_DISPLAY_TYPE // RPi display needs a slower CS change
#define CS_L                                                                   \
...
#define CS_L                                                                   \
  ...
  GPIO.out_w1tc.val = (1 << TFT_CS)
#define CS_H                                                                   \
  GPIO.out_w1ts.val = (1 << TFT_CS) //;GPIO.out_w1ts.val = (1 << TFT_CS)
#endif
#else
#define CS_L
#define CS_H
...
// things up dramatically can then use e.g. GPIO.out_w1ts.val = set_mask(0xFF);
// to set data bus to 0xFF
...
// RAM
#define set_mask(C)                                                            \
  xset_mask[C] // 63fps Sprite rendering test 33% faster, graphicstest only 1.8%
               // faster than shifting in real time
