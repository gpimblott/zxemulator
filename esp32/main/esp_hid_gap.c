...
static const char *gap_bt_prop_type_names[5] = {"", "BDNAME", "COD", "RSSI",
                                                "EIR"};
#endif
...
#if (CONFIG_BT_NIMBLE_ENABLED)
static esp_hid_scan_result_t *find_scan_result(const uint8_t *bda,
                                               esp_hid_scan_result_t *results) {
  esp_hid_scan_result_t *r = results;
  while (r) {
...
#if CONFIG_BT_NIMBLE_ENABLED
static void add_ble_scan_result(const uint8_t *bda, uint8_t addr_type,
                                uint16_t appearance, uint8_t *name,
                                uint8_t name_len, int rssi) {
  if (find_scan_result(bda, ble_scan_results)) {
    ESP_LOGW(TAG, "Result already exists!");
...
static void bt_gap_event_handler(esp_bt_gap_cb_event_t event,
  ...
  case ESP_BT_GAP_KEY_NOTIF_EVT:
    ESP_LOGI(TAG, "BT GAP KEY_NOTIF passkey:%06" PRIu32,
...
esp_err_t esp_hid_ble_gap_adv_init(uint16_t appearance,
  ...
  esp_ble_auth_req_t auth_req =
      ESP_LE_AUTH_BOND; // Bonding only, no MITM (Just Works)
  esp_ble_io_cap_t iocap =
      ESP_IO_CAP_NONE; // No input/output capability = Just Works pairing
...
static void handle_ble_device_result(const struct ble_gap_disc_desc *disc) {
  ...
  for (int i = 0; i < fields.num_uuids16; i++) {
    if (ble_uuid_u16(&fields.uuids16[i].u) == BLE_HID_SVC_UUID &&
        ((adv_name_len > 0 &&
          memcmp("ESP BLE HID2", adv_name, adv_name_len) == 0) ||
         (adv_name_len > 0 &&
          memcmp("ESP Mouse", adv_name, adv_name_len) == 0) ||
         (adv_name_len > 0 &&
          memcmp("ESP Keyboard", adv_name, adv_name_len) == 0))) {
      add_ble_scan_result(disc->addr.val, disc->addr.type, appearance, adv_name,
                          adv_name_len, disc->rssi);
      break;
    }
  }
}
